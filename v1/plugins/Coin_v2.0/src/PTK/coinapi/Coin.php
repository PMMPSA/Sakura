	...
	*/
	...
	public function onEnable(){
		...
		if(is_numeric($time)){
			...
			$this->getLogger()->notice("§aTự Động Lưu Đã Được Đặt Thời Gian Tối Thiểu Thành Khoảng: ".$time." Phút");
		}
		...
		if($update_check->get("check-update")){
			try{
				$this->getLogger()->info("§aKiểm Tra Cập Nhập... Có Thể Mất Một ");
	...
	public function onCommandProcess(CommandSender $sender){
		...
		if($command instanceof Command){
			if($command->testPermissionSilent($sender)){
				$this->save();
				$sender->sendMessage("§4༺§eCoin§4༻ §aĐã Lưu Dữ Liệu Coin.");
			}
		}
	}
