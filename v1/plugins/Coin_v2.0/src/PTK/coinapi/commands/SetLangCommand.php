...
class SetLangCommand extends CoinCommand{
	public function __construct(Coin $plugin, $cmd = "setlangcoin"){
		parent::__construct($plugin, $cmd);
		$this->setUsage("§b/$cmd §f<§aLang§f>");
		$this->setPermission("coinapi.command.setlang");
		$this->setDescription("Chỉnh Sửa Ngôn Ngữ Coin");
	}
	...
	public function exec(CommandSender $sender, array $params){
		...
		if($result === false){
			$sender->sendMessage("§4༺§eCoin§4༻§c Ngôn Ngữ Yêu Cầu Không Tồn Tại");
		}else{
			$sender->sendMessage("§4༺§eCoin§4༻§a Ngôn Ngữ Của Bạn Đã Được Đặt Thành:§b ".$result);
		}
		return true;
	}
}