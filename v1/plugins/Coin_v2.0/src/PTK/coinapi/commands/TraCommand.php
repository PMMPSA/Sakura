...
class TraCommand extends CoinCommand implements InGameCommand{
	public function __construct(Coin $plugin, $cmd = "paycoin"){
		...
		$this->setUsage("§b/$cmd §f<§aTên Người Chơi§f> §f<§aSố Lượng Coin§f>");
		$this->setPermission("coinapi.command.pay");
		$this->setDescription("Chuyển Coin Cho Một Tài Khoản Nào Đó");
	}
	...
	public function exec(CommandSender $sender, array $params){
		...
		if(trim($player) === "" or trim($amount) === "" or !is_numeric($amount)){
			$sender->sendMessage("§4༺§eCoin§4༻ ".$this->getUsage());
			return true;
		}
		...
		if($result !== Coin::RET_SUCCESS){
			$sender->sendMessage($this->getPlugin()->getMessage("pay-success", $sender));
			return true;
		}
