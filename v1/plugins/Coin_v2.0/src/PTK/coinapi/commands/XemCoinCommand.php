...
class XemCoinCommand extends CoinCommand{
	public function __construct(Coin $plugin, $cmd = "seecoin"){
		...
		$this->setUsage("§b/$cmd §f<§aTên Người Chơi§f>");
		$this->setDescription("Kiểm Tra Số Lượng Coin Của Một Tài Khoản Nào Đó");
	...
	public function exec(CommandSender $sender, array $args){
		...
		if(trim($player) === ""){
			$sender->sendMessage("§4༺§eCoin§4༻ §b/".$this->getName()." §f<§aTên Người Chơi§f>");
			return true;
		}
