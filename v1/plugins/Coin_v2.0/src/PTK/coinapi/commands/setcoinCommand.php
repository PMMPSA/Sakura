...
class SetCoinCommand extends CoinCommand{
	public function __construct(Coin $plugin, $cmd = "setcoin"){
		...
		$this->setUsage("§b/$cmd §f<§aNgười Chơi§f> §f<§aSố Lượng Coin§f>");
		$this->setDescription("Chỉnh Sửa Số Coin Của Người Chơi Nào Đó");
	...
	public function exec(CommandSender $sender, array $args){
		...
		if(trim($player) === "" or trim($coin) === ""){
			$sender->sendMessage("§4༺§eCoin§4༻ §b/".$this->getName()." §f<§aNgười Chơi§f> §f<§aSố Lượng Coin§f>");
			return true;
		}
