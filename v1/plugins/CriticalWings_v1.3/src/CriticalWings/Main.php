...
class Main extends PluginBase implements Listener{
	...
	public function onEnable()
	{
		...
		if(!is_file($df . "config.yml")){
			...
			$cfg->setAll([
				"wings-off" => "§b⚔§aCánh Đã §dTắt§a Ghi §e/wingcritical§a Để §dBật§b⚔",
				"wings-on" => "§b⚔§aCánh Đã §dBật§a Ghi §e/wingcritical§a Để §dTắt§b⚔",
				"update-period" => 1
			]);
			$cfg->save();
		}
	...
	public function onCommand(CommandSender $sender, Command $command, $label, array $params)
	{
		...
		if($command->getName() == "wingcritical"){
