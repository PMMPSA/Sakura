...
class CoinAPI extends PluginBase implements Listener{
	...
	public function createAccount($player, $defaultCoin = false, bool $force = false) : bool{
		...
		if(!$this->provider->accountExists($player)){
			...
			$this->getServer()->getPluginManager()->callEvent($ev = new CreateAccountEvent($this, $player, $defaultCoin, "none"));
	...
	public function setCoin($player, $amount, bool $force = false, string $issuer = "none") : int{
		...
		if($this->provider->accountExists($player)){
			...
			$this->getServer()->getPluginManager()->callEvent($ev = new SetCoinEvent($this, $player, $amount, $issuer));
			if(!$ev->isCancelled() or $force === true){
				$this->provider->setCoin($player, $amount);
				$this->getServer()->getPluginManager()->callEvent(new CoinChangedEvent($this, $player, $amount, $issuer));
	...
	public function addCoin($player, $amount, bool $force = false, $issuer = "none") : int{
		...
		if(($coin = $this->provider->getCoin($player)) !== false){
			...
			$this->getServer()->getPluginManager()->callEvent($ev = new AddCoinEvent($this, $player, $amount, $issuer));
			if(!$ev->isCancelled() or $force === true){
				$this->provider->addCoin($player, $amount);
				$this->getServer()->getPluginManager()->callEvent(new CoinChangedEvent($this, $player, $amount + $coin, $issuer));
				return self::RET_SUCCESS;
			}
			return self::RET_CANCELLED;
		}
	...
	public function reduceCoin($player, $amount, bool $force = false, $issuer = "none") : int{
		...
		if(($coin = $this->provider->getCoin($player)) !== false){
			...
			$this->getServer()->getPluginManager()->callEvent($ev = new ReduceCoinEvent($this, $player, $amount, $issuer));
			if(!$ev->isCancelled() or $force === true){
				$this->provider->reduceCoin($player, $amount);
				$this->getServer()->getPluginManager()->callEvent(new CoinChangedEvent($this, $player, $coin - $amount, $issuer));
				return self::RET_SUCCESS;
			}
			return self::RET_CANCELLED;
		}
	...
	private function initialize(){
		...
		switch(strtolower($this->getConfig()->get("provider"))){
			case "yaml":
			$this->provider = new YamlProvider($this->getDataFolder()."Coin.yml");
			break;
			case "mysql":
	...
	private function registerCommands(){
		...
		$commands = [
			...
			"setlangcoin" => "\\onebone\\coinapi\\command\\SetLangCoinCommand",
