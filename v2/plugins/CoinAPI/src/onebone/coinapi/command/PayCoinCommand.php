...
class PayCoinCommand extends Command{
	...
	public function execute(CommandSender $sender, $label, array $params){
		...
		if($result === CoinAPI::RET_SUCCESS){
			...
			$sender->sendMessage($this->plugin->getMessage("paycoin-success", [$amount, $player], $sender->getName()));
			if($p instanceof Player){
				$p->sendMessage($this->plugin->getMessage("coin-paid", [$sender->getName(), $amount], $sender->getName()));
			}
		}else{
			$sender->sendMessage($this->plugin->getMessage("paycoin-failed", [$player, $amount], $sender->getName()));
		}
		return true;
	}
}
